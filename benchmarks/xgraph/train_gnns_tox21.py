...
class TrainModel(object):
    ...
    def __loss__(self, logits, labels):
        return F.cross_entropy(logits, torch.max(labels, 1)[1])
    ...
    def eval(self):
        ...
        if self.graph_classification:
            ...
            for batch in self.loader['eval']:
                ...
                losses.append(loss)
                accs.append(batch_preds == torch.max(batch.y, 1)[1])
    ...
    def test(self):
        ...
        if self.graph_classification:
            ...
            for batch in self.loader['test']:
                batch = batch.to(self.device)
                loss, batch_preds = self._eval_batch(batch, torch.max(batch.y, 1)[1])
                losses.append(loss)
